...
export class Mux {
  ...
  registerComponent(workingArea, x = 0, y = 0) {
    ...
    if (!el) {
      console.error(`Mux element with id ${this.id} not found in DOM`);
      return;
    }
...
// Add a mux to the circuit board
export function addMux(event) {
  let mux = new Mux();
...
export function checkConnectionsMux() {
  ...
  for (let muxId in mux) {
    ...
    if (
      muxComponent.outputIsConnected === false ||
      muxComponent.outputs.length === 0
    ) {
      printErrors("Highlighted component not connected properly\n", id);
      return false;
    }
